...
- v2.78 起，模型详情参数、性能和建议面板标题由共享 `ModelDetailPanelTextLayoutPolicy` 使用公开 Dynamic Type 语义字体，最多两行并允许垂直增长，保留现有 panel 间距、详情行排版、整体/行级辅助语义和模型状态边界；源码测试函数数为 123。本地只做轻量检查，完整 build/test、LogicSmoke 和 Agent C 结果包验收仍待本轮 push 后 GitHub Actions。
...
- v2.78 新增 `testModelDetailPanelTextLayoutPolicySupportsDynamicTypeHeadings`；锁住模型详情面板标题的 2 行上限、1pt line spacing、12pt content spacing、语义 Dynamic Type 契约、重复读取不变性、详情行/详情列辅助语义、44pt 动作与 missing/staged/verified runtime 门禁，并用真实 `ModelParametersPanel`、`ModelPerformancePanel`、`ModelAdvicePanel` 覆盖亮暗主题、320/390/834/1200pt 和 `.large`/`.xxxLarge`/`.accessibility3` 的公开 `ImageRenderer` 非空/正尺寸结果；源码测试函数总数为 123。本地未运行 XCTest、xcodebuild、Simulator、Catalyst、截图或视觉验收。
