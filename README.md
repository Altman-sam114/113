...
- v2.48 新增 `testOptimizerMetricTextLayoutPolicySupportsDynamicTypeCards`，当前测试函数总数为 91。
...
说明：当前 Codex 沙箱内的 CoreSimulator 访问受限；v2.48 本轮未默认重跑本机完整模拟器 XCTest。本轮在工作区内执行 `git diff --check`、`plutil -lint`、workflow YAML 解析、91 个测试函数统计、逻辑烟测和 Swift typecheck；完整 iOS XCTest 与云端 Mac Catalyst 重验证以本轮 push 后的 GitHub Actions run 和 Agent C 下载结果包验收为准。
