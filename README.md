...
- v2.73 新增 `testSessionChipSidebarMetadataPolicyKeepsVerticalRowsScannable`；锁住会话侧栏消息数、数组顺序最近非空摘要、空/空白回退、whitespace/Tab/换行归一化、40 Character 截断、原始会话不变性、vertical/horizontal plan，以及真实 `SessionChip` 在 240/310pt × 亮/暗主题 × `.large`/`.accessibility3` × 选中/未选中下的非 nil/宽度/合理高度渲染断言。当前测试函数总数为 118；完整云端 build/test/LogicSmoke 由本轮 push 后 GitHub Actions 执行，尚待 Agent C 验收。
