...
### v2.77 Agent C 云端验收记录

日期：2026-08-09

- 最新 `origin/main` 为 `7e671e08f78796e5653962b589b1e38e4bf000be`，branch=`main`，subject=`v2.77: 导出会话正文动态排版`。唯一实现 run 为 [31299106692](https://github.com/Altman-sam114/113/actions/runs/31299106692)，`attempt=1`，workflow=`Local Gemma CI Results`，GitHub API conclusion=`success`；run 的 required steps（static、LogicSmoke、iOS build、XCTest、Mac Catalyst build、run-script contract、结果生成、评估和上传）全部 success。仅有 Node.js 20 弃用 annotation。
- GitHub API 只返回一个未过期 artifact：`localgemma-ci-v2.77-main-7e671e0-run31299106692-attempt1`，artifact ID `9033997306`，声明大小 `69508151` bytes，API digest=`sha256:334578f9353194de62aef65daa59f6ff189f276a316f4b034a27f391f11112db`。下载到 `/private/tmp/localgemma-c-review-31299106692/artifact-full.zip` 的 ZIP 大小为 `69508151` bytes，SHA-256 与 API digest 完全一致，`unzip -t` 无错误；`artifact-name.txt` 与 manifest 的 artifact name 完全一致。
- `ci-artifact-manifest.json`、failure summary、JUnit、outcomes、environment、iOS/Catalyst build log、XCTest log、LogicSmoke log、run-script log 和三个 `.xcresult` 均来自该 artifact；manifest 的 repository、branch、version、commit SHA/subject、run ID、attempt、workflow、destination 和结果路径全部与 API identity 对齐。required outcomes 为 `static=success`、`logic=success`、`build=success`、`test=success`、`macCatalyst=success`、`macRunScript=success`；`macDesigned=skipped`、`codexRunEnvironment=skipped` 为既有可选检查，后者原因是 `not-added-in-v1.0-cli-entrypoint-only`。JUnit 为 7 stages、0 failures、1 个预期 skipped；failure summary 明确为 `All required checks passed`。
- 三份 `.xcresult` 的 `Info.plist` 均为 version `3.58`、backend=`fileBacked2`，rootId 存在且 root data/ref 均可读取：iOS build `Data/refs=3/3`，Catalyst build `3/3`，tests `865/865`；三者 data/ref hash key 集合完全配对。iOS/Catalyst root action status 均为 `succeeded`，Catalyst 仅有 root warning summary；tests root 为 `testsCount=122`、action `status=succeeded`。tests bundle 有一个零字节 data 节点，但 refs 配对完整，不影响 bundle 结构。
