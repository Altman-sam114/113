...
读图说明：这张图展示 ContentView 如何根据容器尺寸选择单栏、compact 双栏或 regular 大屏双栏布局，然后进入具体工作区。iPhone 横屏、iPad 竖屏大画布、Mac Catalyst 和桌面窗口都走同一套尺寸断点；regular 大屏侧栏显示工作区用途说明，compact 侧栏保持紧凑；Mac Catalyst 和 iPad 外接键盘可通过 `Command+1...4` 或系统 `工作区` 命令菜单切换工作区，也可通过系统 `会话` 命令菜单或会话栏可见按钮新建/导出当前会话；顶部模型胶囊、模型概要面板、模型详情右栏与行级内容、模型页整体宽屏内容宽度策略、模型详情右栏最大阅读宽度策略、模型详情行文本动态排版策略、模型概要标题文本动态排版策略、模型文件工作流面板、模型文件操作 44pt 触控目标、模型文件动作按钮文本动态排版策略、模型部署控件 44pt 触控目标、模型选择器文本动态排版策略、模型部署电源按钮文本动态排版策略、模型卸载确认弹层、模型状态徽章、全局 Header 图标动作 44pt 触控目标、Header 标题动态排版策略、模型胶囊文本动态排版策略、工作区导航 44pt 触控目标、设置页整体宽屏内容宽度策略、设置页图标动作 44pt 触控目标、设置偏好行文本动态排版策略、会话栏操作 44pt 触控目标、会话 chip 选择/删除动作、会话 chip 选择/删除 44pt 触控目标、会话 chip 标题文本动态排版策略、聊天记录容器、聊天消息气泡、聊天气泡宽屏宽度策略、composer 宽屏输入宽度策略、composer 发送/停止 44pt 触控目标、工作区导航、头部主题与模型库入口、会话栏操作、导出弹层分享/复制辅助语义、导出弹层分享/复制 44pt 触控目标、导出弹层整体宽屏内容宽度策略、导出弹层标题文本动态排版策略、壁纸控件、composer 输入框和发送/停止按钮、模型选择器、部署控件、运行策略开关、运行策略开关宽屏网格、运行策略开关行 44pt 触控目标、芯片准备度、优化指标卡、优化指标卡文本动态排版策略、优化指标网格宽度策略、共享 SectionHeader 动态排版策略、提示词页整体宽屏内容宽度策略、提示词模板宽屏布局策略、提示词模板文本动态排版策略、提示词分类筛选换行布局策略、提示词分类文本动态排版策略、提示词模板动作 44pt 触控目标、提示词分类筛选 chip 和提示词模板动作会向辅助技术暴露当前操作语义。

```mermaid
flowchart TD
    A[ContentView 获取 GeometryReader 尺寸] --> B[WorkspaceLayoutMode.resolve]
    B --> C{是否达到双栏、大屏或桌面窗口断点}
    C -- 否 --> D[单栏布局<br/>Header + TabPicker + Page TabView<br/>Header 图标动作 44pt 触控目标<br/>Header 标题动态排版策略<br/>模型胶囊文本动态排版策略<br/>顶部工作区 tab 44pt 触控目标]
    C -- 是 --> E[双栏布局<br/>左侧状态/导航栏 + 右侧工作区<br/>Header 图标动作 44pt 触控目标<br/>Header 标题动态排版策略<br/>模型胶囊文本动态排版策略<br/>侧栏工作区按钮 44pt 触控目标]
    E --> N{是否 regular 大屏}
